...
from plint.common import APOSTROPHES, CONSONANTS, normalize, is_consonants, is_vowels, SURE_END_FEM, strip_accents_one, \
    strip_accents
import re
from haspirater import haspirater
from plint import error, vowels, common
import sys
...
def elision_wrap(w):
    first_letter = common.remove_punctuation(w[0]['original'].strip())
...
class Verse:
    ...
    def is_empty_word(self, word):
        return re.match("^\s*$", word) or len(normalize(word, rm_all=True)) == 0
